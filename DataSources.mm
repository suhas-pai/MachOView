    ...
    if (selectedNode.details == nil) {
        ...
        if (colIndex == OFFSET_COLUMN) {
            NSString *cellContent =
                [NSString stringWithFormat:@"%.8lX", offset];
            if ([document isRVA] == YES) {
                id layout =
                    [selectedNode.userInfo objectForKey:MVLayoutUserInfoKey];
                return [layout performSelector:@selector(convertToRVA:)
        ...
        memcpy(buffer,
               (uint8_t *)[document.dataController.fileData bytes] + offset,
               len);
...
- (void)tableView:(NSTableView *)aTableView
    ...
    MVDocument *document = [[[aTableView window] windowController] document];
    NSString *cellContent =
        ([anObject isKindOfClass:[NSAttributedString class]] ? [anObject string]
                                                             : anObject);
    ...
    // option1: plain hex value
    {
        ...
        if (dataRange.length <= sizeof(uint64_t)) {
            ...
            [document.dataController.fileData replaceBytesInRange:dataRange
                                                        withBytes:&value];
        } else {
            // create a place holder for new value
            NSAssert([cellContent length] % 2 == 0,
                     @"cell content length must be even");

            NSMutableData *mdata =
                [NSMutableData dataWithCapacity:dataRange.length];
