...
- (MVNode *)createBindingNode:(MVNode *)parent
    ...
    while (NSMaxRange(range) < location + length && isDone == NO) {
        ...
        case BIND_OPCODE_SET_DYLIB_ORDINAL_ULEB:
            ...
            libOrdinal = [dataController read_uleb128:range
                                          lastReadHex:&lastReadHex];
...
- (void)exportSymbol:(uint64_t)address
    ...
    [node.details
        insertRowWithOffset:
                   location:[NSString stringWithFormat:@"%.8X", location
    ]:@"":descStr:symbolName];
...
- (void)printSymbols:(NSString *)prefix
    ...
    if (terminalSize != 0) {
        ...
        if ((flags & EXPORT_SYMBOL_FLAGS_KIND_MASK) ==
            ...
            [node.details insertRowWithOffset:range.location:@"":@"":@"00":@"EXPORT_SYMBOL_FLAGS_KIND_REGULAR"];
        if ((flags & EXPORT_SYMBOL_FLAGS_KIND_MASK) ==
            EXPORT_SYMBOL_FLAGS_KIND_THREAD_LOCAL)
            [node.details insertRowWithOffset:range.location:@"":@"":@"01":@"EXPORT_SYMBOL_FLAGS_KIND_THREAD_LOCAL"];
        if (flags & EXPORT_SYMBOL_FLAGS_WEAK_DEFINITION)
            [node.details insertRowWithOffset:range.location:@"":@"":@"04":@"EXPORT_SYMBOL_FLAGS_WEAK_DEFINITION"];
        if (flags & EXPORT_SYMBOL_FLAGS_REEXPORT)
            [node.details insertRowWithOffset:range.location:@"":@"":@"08":@"EXPORT_SYMBOL_FLAGS_REEXPORT"];
        if (flags & EXPORT_SYMBOL_FLAGS_STUB_AND_RESOLVER)
            [node.details insertRowWithOffset:range.location:@"":@"":@"10":@"EXPORT_SYMBOL_FLAGS_STUB_AND_RESOLVER"];
