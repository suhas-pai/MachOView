...
- (MVNode *)createPointersNode:(MVNode *)parent
    ...
    while (NSMaxRange(range) < location + length) {
        uint32_t ptr = [dataController read_uint32:range
                                       lastReadHex:&lastReadHex];
        NSString *symbolName = [NSString
                                stringWithFormat:
                                @"%@->%@",
                                [self findSymbolAtRVA:
        ...
        [node.details
         ...
         ]:lastReadHex:@"Pointer":symbolName];

        [node.details setAttributes:MVMetaDataAttributeName, symbolName, nil];

        [symbolNames
         setObject:symbolName
            forKey:[NSNumber numberWithUnsignedLong:
                    [self fileOffsetToRVA:
...
- (MVNode *)createPointers64Node:(MVNode *)parent
    ...
    while (NSMaxRange(range) < location + length) {
        uint64_t ptr = [dataController read_uint64:range
                                       lastReadHex:&lastReadHex];
        NSString *symbolName = [NSString
                                stringWithFormat:@"%@->%@",
                                [self findSymbolAtRVA64:
...
- (MVNode *)createCStringsNode:(MVNode *)parent
    ...
    while (NSMaxRange(range) < location + length) {
        NSString *symbolName = [dataController read_string:range
                                               lastReadHex:&lastReadHex];

        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:[NSString stringWithFormat:@"CString (length:%lu)",
                        [symbolName length]
         ]:symbolName];
        ...
        if ([self is64bit] == NO) {
            ...
            [symbolNames setObject:[NSString stringWithFormat:@"0x%X:\"%@\"",
                                    rva, symbolName]
                            forKey:[NSNumber numberWithUnsignedLong:rva]];
        } else {
            uint64_t rva64 = [self fileOffsetToRVA64:range.location];
            [symbolNames setObject:[NSString stringWithFormat:@"0x%qX:\"%@\"",
                                    rva64, symbolName]
...
- (MVNode *)createLiteralsNode:(MVNode *)parent
    ...
    while (NSMaxRange(range) < location + length) {
        NSData *data = [dataController read_bytes:range
                                           length:stride
                                      lastReadHex:&lastReadHex];
        ...
        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:@"Floating Point Number":literalStr];

        // fill in lookup table with string literals
        if ([self is64bit] == NO) {
            uint32_t rva = [self fileOffsetToRVA:(uint32_t)range.location];
            [symbolNames setObject:[NSString stringWithFormat:@"0x%X:%@f", rva,
                                    literalStr]
                            forKey:[NSNumber numberWithUnsignedLong:rva]];
        } else {
            uint64_t rva64 = [self fileOffsetToRVA64:range.location];
            [symbolNames setObject:[NSString stringWithFormat:@"0x%qX:%@f",
                                    rva64, literalStr]
                            forKey:[NSNumber numberWithUnsignedLongLong:rva64]];
        }
    }
...
- (MVNode *)createIndPointersNode:(MVNode *)parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:@"Indirect Pointer":symbolName];
...
- (MVNode *)createIndStubsNode:(MVNode *)parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:@"Indirect Stub":symbolName];
...
//============================= X86 =========================
//===========================================================
__unused static AsmFootPrint const classicStubHelperX86 = {
    {
        1, 0x68
    },
...
__unused static AsmFootPrint const hybridStubHelperX86 = {
    {
        1, 0x68
    }, GAP(4),         // pushl   $lazy-info-offset
    {
        1, 0x68
    }, GAP(4),         // pushl   $foo$lazy_ptr
    {
...
__unused static AsmFootPrint const hybridStubHelperHelperX86 = {
    {
        2, 0x83, 0x3D
    },
    GAP(4),
...
__unused static AsmFootPrint const fastStubHelperX86 = {
    {
        1, 0x68
    },
    GAP(4), // pushl   $lazy-info-offset
...
__unused static AsmFootPrint const fastStubHelperHelperX86 = {
    {
        1, 0x68
    },       GAP(4),         // pushl   imageloadercache
    {
...
__unused static AsmFootPrint const classicStubHelperX86_64 = {
    {
        3, 0x4C, 0x8D, 0x1D
    },
...
__unused static AsmFootPrint const hybridStubHelperX86_64 = {
    {
        1, 0x68
    },
    GAP(4), // pushq  $lazy-info-offset
    {
        3, 0x4C, 0x8D, 0x1D
    },
    GAP(4), // lea    foo$lazy_ptr(%rip),%r11
...
__unused static AsmFootPrint const hybridStubHelperHelperX86_64 = {
    {
        3, 0x48, 0x83, 0x3D
    },
    GAP(4),
...
__unused static AsmFootPrint const fastStubHelperX86_64 = {
    {
        1, 0x68
    },
    GAP(4), // pushq  $lazy-info-offset
...
__unused static AsmFootPrint const fastStubHelperHelperX86_64 = {
    {
        3, 0x4C, 0x8D, 0x1D
    },
    GAP(4),          // leaq   dyld_mageLoaderCache(%rip),%r11
    {
        2, 0x41, 0x53
    },               // pushq  %r11
    {
...
//=========================== ARM ===========================
//===========================================================
__unused static AsmFootPrint const fastStubHelperARM = {
    {
        4, 0xe5, 0x9f, 0xc0, 0x00
    },                           // ldr  ip, [pc, #0]
    {
        4, 0xea, 0x00, 0x00, 0x00
    },                           // b	_helperhelper
    GAP(4),                      // lazy binding info
};

__unused static AsmFootPrint const fastStubHelperHelperARM = {
    // push lazy-info-offset
    {
        4, 0xe5, 0x2d, 0xc0, 0x04
    },                           // str ip, [sp, #-4]!
                                 // push address of dyld_mageLoaderCache
    {
        4, 0xe5, 0x9f, 0xc0, 0x10
    },                           // ldr	ip, L1
    {
        4, 0xe0, 0x8f, 0xc0, 0x0c
    },                           // add	ip, pc, ip
    {
        4, 0xe5, 0x2d, 0xc0, 0x04
    },                           // str ip, [sp, #-4]!
    ...
    {
        4, 0xe5, 0x9f, 0xc0, 0x08
    },                           // ldr	ip, L2
    {
        4, 0xe0, 0x8f, 0xc0, 0x0c
    },                           // add	ip, pc, ip
    {
        4, 0xe5, 0x9c, 0xf0, 0x00
    },                           // ldr	pc, [ip]
...
- (MVNode *)createStubHelperNode:(MVNode *)parent
    ...
    if ([self matchAsmAtOffset:range.location
                  asmFootPrint:hybridStubHelperHelperX86
                     lineCount:sizeof(hybridStubHelperHelperX86) /
         FOOTPRINT_STRIDE]) {
        data = [dataController read_bytes:range
                                   length:7
                              lastReadHex:&lastReadHex];
        address = *(uint32_t *)((uint8_t *)data.bytes + 2);
        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:@"cmpl  $0x00,_fast_lazy_bind"
                  :[self findSymbolAtRVA:address]];

        [dataController read_bytes:range length:2 lastReadHex:&lastReadHex];
        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:@"jne  $0x0D":@""];

        [dataController read_bytes:range length:4 lastReadHex:&lastReadHex];
        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:@"movl  %eax,4(%esp)":@""];

        [dataController read_bytes:range length:1 lastReadHex:&lastReadHex];
        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:@"popl  %eax":@""];

        [dataController read_bytes:range length:3 lastReadHex:&lastReadHex];
        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:@"xchgl  (%esp),%eax":@""];

        data = [dataController read_bytes:range
                                   length:5
                              lastReadHex:&lastReadHex];
        address =
            [self fileOffsetToRVA:(uint32_t)NSMaxRange(range) +
             *(uint32_t *)((uint8_t *)data.bytes + 1)];
        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:@"jmpl  dyld_stub_binding_helper"
                  :[self findSymbolAtRVA:address]];

        [dataController read_bytes:range length:3 lastReadHex:&lastReadHex];
        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:@"addl  $0x04,%esp":@""];
        ...
        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:@"pushl  imageloadercache"
                  :[self findSymbolAtRVA:address]];
        ...
        [node.details
         appendRow:[NSString stringWithFormat:@"%.8lX", range.location
         ]:lastReadHex:@"jmp  *_fast_lazy_bind":[self findSymbolAtRVA:address]];
...
- (MVNode *)createTextNode:(MVNode *)parent
    ...
    uint64_t ot_addr =
        ([self is64bit] == NO ? [self fileOffsetToRVA:location]
                              : [self fileOffsetToRVA64:location]);
    ...
    cs_err cserr;
    /* open capstone */
    cs_arch target_arch = (cs_arch)0;
    cs_mode target_mode = (cs_mode)0;
    ...
    if ((cserr = cs_open(target_arch, target_mode, &cs_handle)) != CS_ERR_OK) {
        NSLog(@"Failed to initialize Capstone: %d, %s.", cserr,
              cs_strerror(cs_errno(cs_handle)));
        return node;
    }
    ...
    /* this will fail if we have data in code or jump tables because Capstone
     * stops when it can't disassemble */
    /* a bit of a problem with most binaries :( */
    /* XXX: parse data in code section to partially solve this */
    disasm_count = cs_disasm(cs_handle, (const uint8_t *)ot_sect, ot_left,
                             ot_addr, 0, &cs_insn);
    NSLog(@"Disassembled %lu instructions.", disasm_count);
    uint32_t fileOffset =
        ([self is64bit] == NO ? [self RVAToFileOffset:(uint32_t)ot_addr]
                              : [self RVA64ToFileOffset:ot_addr]);
    for (size_t i = 0; i < disasm_count; i++) {
        /* XXX: replace this bytes retrieval with Capstone internal data since
         * it already contains this info */
        NSRange range = NSMakeRange(fileOffset, 0);
        NSString *lastReadHex;
        [dataController read_bytes:range
                            length:cs_insn[i].size
                       lastReadHex:&lastReadHex];
        /* format the disassembly output using Capstone strings */
        NSString *asm_string =
            [NSString stringWithFormat:@"%-10s\t%s", cs_insn[i].mnemonic,
             cs_insn[i].op_str];
        [node.details appendRow:[NSString stringWithFormat:@"%.8X", fileOffset
         ]:lastReadHex:asm_string:@""];
