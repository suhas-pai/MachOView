...
- (uint8_t)read_uint8:(NSRange &)range lastReadHex:(NSString **)lastReadHex {
    ...
    if (lastReadHex)
        *lastReadHex = [NSString stringWithFormat:@"%.2X", (0xFF & buffer)];
...
- (uint16_t)read_uint16:(NSRange &)range lastReadHex:(NSString **)lastReadHex {
    ...
    [fileData getBytes:&buffer range:range];
    if (lastReadHex)
        *lastReadHex = [NSString stringWithFormat:@"%.4X", (0xFFFF & buffer)];
    [realData getBytes:&buffer range:range];
    return buffer;
}
...
- (uint64_t)read_uint64:(NSRange &)range lastReadHex:(NSString **)lastReadHex {
    ...
    [fileData getBytes:&buffer range:range];
    if (lastReadHex)
        *lastReadHex = [NSString stringWithFormat:@"%.16qX", buffer];
    [realData getBytes:&buffer range:range];
    return buffer;
}
...
- (NSString *)getHexStr:(NSRange &)range {
    NSMutableString *lastReadHex =
        [NSMutableString stringWithCapacity:2 * range.length];
...
- (NSString *)replaceEscapeCharsInString:(NSString *)orig {
    ...
    unichar (*charAtIdx)(id, SEL, NSUInteger) =
        (typeof(charAtIdx))[orig methodForSelector:sel];
...
//-----------------------------------------------------------------------------
- (NSData *)read_bytes:(NSRange &)range
