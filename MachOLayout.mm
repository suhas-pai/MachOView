...
//-----------------------------------------------------------------------------
+ (MachOLayout *)layoutWithDataController:(MVDataController *)dc
...
- (struct section const *)getSectionByIndex:(uint32_t)index {
    static const struct section notfound = {"???", "?????", 0, 0, 0, 0,
                                            0,     0,       0, 0, 0};
...
- (struct section_64 const *)getSection64ByIndex:(uint32_t)index {
    static const struct section_64 notfound = {"???", "?????", 0, 0, 0, 0,
                                               0,     0,       0, 0, 0};
...
- (NSString *)findSymbolAtRVA:(uint32_t)rva {
    ...
    NSString *symbolName =
        [symbolNames objectForKey:[NSNumber numberWithUnsignedLong:rva]];
    return (symbolName != nil ? symbolName
                              : [NSString stringWithFormat:@"0x%X", rva]);
}
...
- (NSString *)findSymbolAtRVA64:(uint64_t)rva64 {
    ...
    NSString *symbolName =
        [symbolNames objectForKey:[NSNumber numberWithUnsignedLongLong:rva64]];
    return (symbolName != nil ? symbolName
                              : [NSString stringWithFormat:@"0x%qX", rva64]);
}
...
- (struct section_64 const *)findSection64ByName:(char const *)sectname
    ...
    for (Section64Vector::const_iterator sectIter = ++sections_64.begin();
        ...
        if ((segname == NULL ||
             strncmp(section_64->segname, segname, 16) == 0) &&
...
- (uint32_t)RVAToFileOffset:(uint32_t)rva {
    ...
    uint32_t fileOffset =
        sectOffset + (rva - [self fileOffsetToRVA:sectOffset]);
    NSAssert1(fileOffset < [dataController.fileData length],
              @"rva is out of range (0x%X)", rva);
    return fileOffset;
}
...
- (uint32_t)RVA64ToFileOffset:(uint64_t)rva64 {
    ...
    uint32_t sectOffset = sectIter->second.first;
    uint32_t fileOffset =
        sectOffset + (rva64 - [self fileOffsetToRVA64:sectOffset]);
    NSAssert1(fileOffset < [dataController.fileData length],
              @"rva is out of range (0x%qX)", rva64);
    return fileOffset;
}
...
- (void)addRelocAtFileOffset:(uint32_t)offset
    ...
    [dataController.realData replaceBytesInRange:NSMakeRange(offset, length)
                                       withBytes:&value];
}

// ----------------------------------------------------------------------------
static inline uint32_t _hex2int(char const *a, uint32_t len) {
    uint32_t val = 0;
...
- (NSString *)convertToRVA:(NSString *)offsetStr {
    ...
    /*
    BOOL scanResult = [[NSScanner scannerWithString:offsetStr]
    ...
    if (scanResult == NO || segmentInfo.empty() ||
        ...
        fileOffset + 1 >= (--segmentInfo.end())->first +
    (--segmentInfo.end())->second.second)
    {
      return @"";
    }
    */
    ...
    if (segmentInfo.empty() || fileOffset < segmentInfo.begin()->first ||
        fileOffset + 1 >= (--segmentInfo.end())->first +
                              (--segmentInfo.end())->second.second) {
        return @"";
    }
    ...
    return (
        [self is64bit] == NO
            ? [NSString
                  stringWithFormat:@"%.8X", [self fileOffsetToRVA:fileOffset]]
            : [NSString stringWithFormat:@"%.8qX",
                                         [self fileOffsetToRVA64:fileOffset]]);
}
...
- (NSDictionary *)userInfoForSection:(struct section const *)section {
    ...
    return [NSDictionary
        dictionaryWithObjectsAndKeys:weakSelf, MVLayoutUserInfoKey,
                                     NSSTRING(
                                     ...
                                     @"sectname",
                                     [NSNumber
                                         numberWithUnsignedLong:section->addr],
                                     @"address", nil];
}
...
- (NSDictionary *)userInfoForRelocs {
    ...
    return [NSDictionary
        dictionaryWithObjectsAndKeys:weakSelf, MVLayoutUserInfoKey,
                                     @"Relocations", MVNodeUserInfoKey, nil];
}
...
- (void)processLinkEdit {
    ...
    for (CommandVector::const_iterator cmdIter = commands.begin();
         cmdIter != commands.end(); ++cmdIter) {
        struct load_command const *load_command = *cmdIter;
        switch (load_command->cmd) {
        case LC_SEGMENT: {
            struct segment_command const *segment_command =
                (struct segment_command const *)load_command;

            if (segment_command->fileoff == 0 &&
        ...
        case LC_TWOLEVEL_HINTS:
            twolevel_hints_command =
                (struct twolevel_hints_command const *)load_command;
            break;
        case LC_SEGMENT_SPLIT_INFO:
            segment_split_info =
                (struct linkedit_data_command const *)load_command;
            break;
    ...
    if (dysymtab_command) {
        ...
        if (dysymtab_command->tocoff > 0) {
            NSRange range =
                NSMakeRange(dysymtab_command->tocoff + imageOffset,
                            dysymtab_command->ntoc *
                                sizeof(struct dylib_table_of_contents));
            dysymtabRange = NSMaxRange(dysymtabRange) > 0
                                ? NSUnionRange(dysymtabRange, range)
                                : range;
        }
        if (dysymtab_command->modtaboff > 0) {
            NSRange range = NSMakeRange(
                dysymtab_command->modtaboff + imageOffset,
                dysymtab_command->nmodtab * sizeof(struct dylib_module));
            dysymtabRange = NSMaxRange(dysymtabRange) > 0
                                ? NSUnionRange(dysymtabRange, range)
                                : range;
        }
        if (dysymtab_command->extrefsymoff > 0) {
            NSRange range = NSMakeRange(
                dysymtab_command->extrefsymoff + imageOffset,
        ...
        if (dysymtab_command->indirectsymoff > 0) {
            NSRange range =
                NSMakeRange(dysymtab_command->indirectsymoff + imageOffset,
                            dysymtab_command->nindirectsyms * sizeof(uint32_t));
            dysymtabRange = NSMaxRange(dysymtabRange) > 0
                                ? NSUnionRange(dysymtabRange, range)
                                : range;
        }
        if (dysymtab_command->extreloff > 0) {
            NSRange range = NSMakeRange(
                dysymtab_command->extreloff + imageOffset,
                dysymtab_command->nextrel * sizeof(struct relocation_info));
            dysymtabRange = NSMaxRange(dysymtabRange) > 0
                                ? NSUnionRange(dysymtabRange, range)
                                : range;
        }
        if (dysymtab_command->locreloff > 0) {
            NSRange range = NSMakeRange(
                dysymtab_command->locreloff + imageOffset,
                dysymtab_command->nlocrel * sizeof(struct relocation_info));
            dysymtabRange = NSMaxRange(dysymtabRange) > 0
                                ? NSUnionRange(dysymtabRange, range)
                                : range;
        }
        ...
        twoLevelHintsNode =
            [self createDataNode:rootNode
                        ...
                        location:twolevel_hints_command->offset + imageOffset
                          length:twolevel_hints_command->nhints *
                                 sizeof(struct twolevel_hint)];
    }

    if (segment_split_info) {
        segmentSplitInfoNode =
            [self createDataNode:rootNode
    ...
    if (dysymtabNode) {
        @try {
            ...
            if (dysymtab_command->tocoff * dysymtab_command->ntoc > 0) {
                [self createTOCNode:dysymtabNode
                           ...
                           location:dysymtab_command->tocoff + imageOffset
                             length:dysymtab_command->ntoc *
                                    sizeof(struct dylib_table_of_contents)];
            }

            //======= External Reference Table =============
            //==============================================
            if (dysymtab_command->extrefsymoff * dysymtab_command->nextrefsyms >
                0) {
                [self createReferencesNode:dysymtabNode
                                   caption:(lastNodeCaption =
                                                @"External References")
                                  location:dysymtab_command->extrefsymoff +
                                           imageOffset
                                    length:dysymtab_command->nextrefsyms *
                                           sizeof(struct dylib_reference)];
            }

            //========== Indirect Symbol Table =============
            //==============================================
            if (dysymtab_command->indirectsymoff *
                    dysymtab_command->nindirectsyms >
                0) {
                [self createISymbolsNode:dysymtabNode
                                 caption:(lastNodeCaption = @"Indirect Symbols")
                                location:dysymtab_command->indirectsymoff +
                                         imageOffset
                                  length:dysymtab_command->nindirectsyms *
                                         sizeof(uint32_t)];
            }
            ...
            if (dysymtab_command->extreloff * dysymtab_command->nextrel > 0) {
                [self
                    createRelocNode:dysymtabNode
                            caption:(lastNodeCaption = @"External Relocations")
                           location:dysymtab_command->extreloff + imageOffset
                             length:dysymtab_command->nextrel *
                                    sizeof(struct relocation_info)
                        baseAddress:(mach_header->flags & MH_SPLIT_SEGS) ==
                                            MH_SPLIT_SEGS
            ...
            if (dysymtab_command->locreloff * dysymtab_command->nlocrel > 0) {
                [self createRelocNode:dysymtabNode
                             ...
                             location:dysymtab_command->locreloff + imageOffset
                               length:dysymtab_command->nlocrel *
                                      sizeof(struct relocation_info)
                          baseAddress:(mach_header->flags & MH_SPLIT_SEGS) ==
                                              MH_SPLIT_SEGS
                                          ? segs_read_write_addr
    ...
    if (segmentSplitInfoNode && segmentSplitInfoNode.dataRange.length > 0) {
        @try {
            [self
                createSplitSegmentNode:segmentSplitInfoNode
    ...
    if (dataInCodeEntriesNode && dataInCodeEntriesNode.dataRange.length > 0) {
        @try {
            [self createDataInCodeEntriesNode:dataInCodeEntriesNode
                                     ...
                                     location:dataInCodeEntriesNode.dataRange
                                                  .location
                                       length:dataInCodeEntriesNode.dataRange
                                                  .length];
        } @catch (NSException *exception) {
            [self printException:exception caption:lastNodeCaption];
        }
...
- (void)processLinkEdit64 {
    ...
    for (CommandVector::const_iterator cmdIter = commands.begin();
        ...
        case LC_SEGMENT_64: {
            struct segment_command_64 const *segment_command_64 =
                (struct segment_command_64 const *)load_command;

            if (segment_command_64->fileoff == 0 &&
                segment_command_64->filesize != 0) {
    ...
    if (dysymtab_command) {
        ...
        if (dysymtab_command->modtaboff > 0) {
            NSRange range = NSMakeRange(
                dysymtab_command->modtaboff + imageOffset,
                dysymtab_command->nmodtab * sizeof(struct dylib_module_64));
            dysymtabRange = NSMaxRange(dysymtabRange) > 0
                                ? NSUnionRange(dysymtabRange, range)
                                : range;
        }
        ...
        @try {
            ...
            if (dysymtab_command->modtaboff * dysymtab_command->nmodtab > 0) {
                [self createModules64Node:dysymtabNode
                                 ...
                                 location:dysymtab_command->modtaboff +
                                          imageOffset
                                   length:dysymtab_command->nmodtab *
            ...
            if (dysymtab_command->indirectsymoff *
                ...
                0) {
                [self
                    createISymbols64Node:dysymtabNode
                                 caption:(lastNodeCaption = @"Indirect Symbols")
                                location:dysymtab_command->indirectsymoff +
                                         imageOffset
                                  length:dysymtab_command->nindirectsyms *
            ...
            if (dysymtab_command->extreloff * dysymtab_command->nextrel > 0) {
                [self
                    createReloc64Node:dysymtabNode
                              caption:(lastNodeCaption =
                                           @"External Relocations")
                             location:dysymtab_command->extreloff + imageOffset
                               length:dysymtab_command->nextrel *
                                      sizeof(struct relocation_info)
                          baseAddress:(mach_header_64->flags & MH_SPLIT_SEGS) ==
                                              MH_SPLIT_SEGS
                                          ? segs_read_write_addr
                                          : seg1addr];
            }
...
- (void)processDyldInfo {
    ...
    if (dyld_info_command->rebase_off > 0) {
        NSRange range = NSMakeRange(dyld_info_command->rebase_off + imageOffset,
                                    dyld_info_command->rebase_size);
        dyldInfoRange = NSMaxRange(dyldInfoRange) > 0
                            ? NSUnionRange(dyldInfoRange, range)
                            : range;
    }
    if (dyld_info_command->bind_off > 0) {
        NSRange range = NSMakeRange(dyld_info_command->bind_off + imageOffset,
    ...
    if (dyld_info_command->weak_bind_off > 0) {
        NSRange range =
            NSMakeRange(dyld_info_command->weak_bind_off + imageOffset,
    ...
    if (dyld_info_command->lazy_bind_off > 0) {
        NSRange range =
            NSMakeRange(dyld_info_command->lazy_bind_off + imageOffset,
    ...
    if (dyld_info_command->export_off > 0) {
        NSRange range = NSMakeRange(dyld_info_command->export_off + imageOffset,
    ...
    DyldHelper *dyldHelper = [DyldHelper dyldHelperWithSymbols:symbolNames
                                                       is64Bit:[self is64bit]];

    NSString *lastNodeCaption;
    @try {
        if (dyld_info_command->rebase_off * dyld_info_command->rebase_size >
            0) {
        ...
        if (dyld_info_command->weak_bind_off *
                dyld_info_command->weak_bind_size >
            0) {
            [self
                createBindingNode:dyldInfoNode
        ...
        if (dyld_info_command->lazy_bind_off *
                dyld_info_command->lazy_bind_size >
            0) {
            [self
                createBindingNode:dyldInfoNode
        ...
        if (dyld_info_command->export_off * dyld_info_command->export_size >
            0) {
...
//-----------------------------------------------------------------------------
template <typename SectionT> struct CompareSectionByName {
    ...
    bool operator()(SectionT const *section) {
        return (
            (segname == NULL || strncmp(segname, section->segname, 16) == 0) &&
...
- (void)processSections {
    ...
    for (SectionVector::const_iterator sectIter = ++sections.begin();
        ...
        struct section const *section = *sectIter;
        MVNode *sectionNode =
            [self findNodeByUserInfo:[self userInfoForSection:section]];
        if (sectionNode == nil) {
            continue;
        }
        ...
        @try {
            ...
            case S_CSTRING_LITERALS:
                [self
                    createCStringsNode:sectionNode
...
- (void)processSections64 {
    ...
    for (Section64Vector::const_iterator sectIter = ++sections_64.begin();
        ...
        struct section_64 const *section_64 = *sectIter;
        MVNode *sectionNode =
            [self findNodeByUserInfo:[self userInfoForSection64:section_64]];
        if (sectionNode == nil) {
            continue;
        }
    ...
    for (Section64Vector::const_iterator sectIter = ++sections_64.begin();
        ...
        @try {
            ...
            case S_LITERAL_POINTERS:
                [self
                    createPointers64Node:sectionNode
                                 caption:(lastNodeCaption = @"Literal Pointers")
                                location:section_64->offset + imageOffset
                                  length:section_64->size];
                break;
            ...
            case S_LAZY_DYLIB_SYMBOL_POINTERS:
                [self
                    createIndPointers64Node:sectionNode
                                    caption:(lastNodeCaption =
                                                 @"Lazy Dylib Symbol Pointers")
                                   location:section_64->offset + imageOffset
                                     length:section_64->size];
...
- (void)processEHFrames {
    ...
    for (SectionVector::iterator sectIter =
             find_if(++sections.begin(), sections.end(),
                     CompareSectionByName<struct section>("__eh_frame"));
         sectIter != sections.end();
         sectIter =
             find_if(++sectIter, sections.end(),
                     CompareSectionByName<struct section>("__eh_frame"))) {
        struct section const *section = *sectIter;
         ...
         * (CFI) records. The number of records present shall be determined by
         * size of the section as contained in the section header. Each CFI
         * record contains a Common Information Entry (CIE) record followed by 1
         * or more Frame Description Entry (FDE) records. Both CIEs and FDEs
        ...
        @try {
            ...
            do {
                ...
                if (cieID == 0) {
                    ...
                    [self
                        createCFINode:sectionNode
                              caption:(lastNodeCaption = [NSString
                                           stringWithFormat:
                                               @"Call Frame %@",
                                               [self findSymbolAtRVA:CIE_addr]])
                             location:location
                               length:section->offset + imageOffset +
                                      section->size - location]; // upper bound
                }
...
- (void)processEHFrames64 {
    ...
    for (Section64Vector::iterator sectIter =
             find_if(++sections_64.begin(), sections_64.end(),
                     CompareSectionByName<struct section_64>("__eh_frame"));
         sectIter != sections_64.end();
         sectIter =
             find_if(++sectIter, sections_64.end(),
                     CompareSectionByName<struct section_64>("__eh_frame"))) {
        struct section_64 const *section_64 = *sectIter;
                        ...
                        createCFINode:sectionNode
                              caption:(lastNodeCaption = [NSString
                                           stringWithFormat:
                                               @"Call Frame %@",
                                               [self
                                                   findSymbolAtRVA64:CIE_addr]])
                             location:location
                               length:section_64->offset + imageOffset +
                                      section_64->size -
                                      location]; // upper bound
                }
                location += length + /*length itself */ sizeof(uint32_t);
            } while (location - section_64->offset - imageOffset <
                     section_64->size);
        } @catch (NSException *exception) {
            [self printException:exception caption:lastNodeCaption];
        }
...
- (void)processLSDA {
    ...
    for (SectionVector::iterator sectIter =
             find_if(++sections.begin(), sections.end(),
                     CompareSectionByName<struct section>("__gcc_except_tab"));
         sectIter != sections.end();
         sectIter = find_if(
             ++sectIter, sections.end(),
             CompareSectionByName<struct section>("__gcc_except_tab"))) {
        ...
        @try {
            for (ExceptionFrameMap::iterator ehFrameIter = lsdaInfo.begin();
                 ehFrameIter != lsdaInfo.end();) {
...
- (void)processLSDA64 {
    ...
    for (Section64Vector::iterator sectIter = find_if(
             ++sections_64.begin(), sections_64.end(),
             CompareSectionByName<struct section_64>("__gcc_except_tab"));
         sectIter != sections_64.end();
         sectIter = find_if(
             ++sectIter, sections_64.end(),
             CompareSectionByName<struct section_64>("__gcc_except_tab"))) {
        ...
        @try {
            for (ExceptionFrameMap::iterator ehFrameIter = lsdaInfo.begin();
                ...
                [self
                    createLSDANode:sectionNode
                           caption:(lastNodeCaption = [NSString
                                        stringWithFormat:
                                            @"LSDA %@",
                                            [self findSymbolAtRVA64:lsdaAddr]])
                          location:location
                            length:length
                    eh_frame_begin:frameAddr];
            }
...
- (void)processObjcSections {
    ...
    @try {
        ...
        if (hasObjCModules == false) {
            section = [self findSectionByName:"__category_list"
                                   andSegment:"__OBJC2"];
            if (section == NULL)
                section = [self findSectionByName:"__objc_catlist"
                                       andSegment:"__DATA"];
            if ((sectionNode = [self
                     findNodeByUserInfo:[self userInfoForSection:section]])) {
            ...
            section = [self findSectionByName:"__class_list"
                                   andSegment:"__OBJC2"];
            if (section == NULL)
                section = [self findSectionByName:"__objc_classlist"
                                       andSegment:"__DATA"];
            if ((sectionNode = [self
                     findNodeByUserInfo:[self userInfoForSection:section]])) {
                [self createObjC2PointerListNode:sectionNode
            ...
            section = [self findSectionByName:"__class_refs"
                                   andSegment:"__OBJC2"];
            if (section == NULL)
                section = [self findSectionByName:"__objc_classrefs"
                                       andSegment:"__DATA"];
            if ((sectionNode = [self
                     findNodeByUserInfo:[self userInfoForSection:section]])) {
                [self createObjC2PointerListNode:sectionNode
            ...
            section = [self findSectionByName:"__super_refs"
                                   andSegment:"__OBJC2"];
            if (section == NULL)
                section = [self findSectionByName:"__objc_superrefs"
                                       andSegment:"__DATA"];
            if ((sectionNode = [self
                     findNodeByUserInfo:[self userInfoForSection:section]])) {
                [self createObjC2PointerListNode:sectionNode
            ...
            section = [self findSectionByName:"__protocol_list"
                                   andSegment:"__OBJC2"];
            if (section == NULL)
                section = [self findSectionByName:"__objc_protolist"
                                       andSegment:"__DATA"];
            if ((sectionNode = [self
                     findNodeByUserInfo:[self userInfoForSection:section]])) {
                [self createObjC2PointerListNode:sectionNode
            ...
            section = [self findSectionByName:"__message_refs"
                                   andSegment:"__OBJC2"];
            if (section == NULL)
                section = [self findSectionByName:"__objc_msgrefs"
                                       andSegment:"__DATA"];
            if ((sectionNode = [self
                     findNodeByUserInfo:[self userInfoForSection:section]])) {
                [self createObjC2MsgRefsNode:sectionNode
                                     caption:(lastNodeCaption =
                                                  @"ObjC2 Message References")
                                    location:section->offset + imageOffset
                                      length:section->size];
            }
        } // if (hasObjcModules == false)
        ...
        if (section == NULL)
            section = [self findSectionByName:"__objc_imageinfo"
                                   andSegment:"__DATA"];
        if ((sectionNode =
                 [self findNodeByUserInfo:[self userInfoForSection:section]])) {
            [self
                createObjCImageInfoNode:sectionNode
...
- (void)processObjcSections64 {
    ...
    @try {
        section_64 = [self findSection64ByName:"__class_list"
                                    andSegment:"__OBJC2"];
        if (section_64 == NULL)
            section_64 = [self findSection64ByName:"__objc_classlist"
                                        andSegment:"__DATA"];
        if ((sectionNode = [self
                 findNodeByUserInfo:[self userInfoForSection64:section_64]])) {
        ...
        section_64 = [self findSection64ByName:"__class_refs"
                                    andSegment:"__OBJC2"];
        if (section_64 == NULL)
            section_64 = [self findSection64ByName:"__objc_classrefs"
                                        andSegment:"__DATA"];
        if ((sectionNode = [self
                 findNodeByUserInfo:[self userInfoForSection64:section_64]])) {
            [self createObjC2Pointer64ListNode:sectionNode
        ...
        section_64 = [self findSection64ByName:"__super_refs"
                                    andSegment:"__OBJC2"];
        if (section_64 == NULL)
            section_64 = [self findSection64ByName:"__objc_superrefs"
                                        andSegment:"__DATA"];
        if ((sectionNode = [self
                 findNodeByUserInfo:[self userInfoForSection64:section_64]])) {
            [self createObjC2Pointer64ListNode:sectionNode
        ...
        section_64 = [self findSection64ByName:"__category_list"
                                    andSegment:"__OBJC2"];
        if (section_64 == NULL)
            section_64 = [self findSection64ByName:"__objc_catlist"
                                        andSegment:"__DATA"];
        if ((sectionNode = [self
                 findNodeByUserInfo:[self userInfoForSection64:section_64]])) {
            [self createObjC2Pointer64ListNode:sectionNode
        ...
        section_64 = [self findSection64ByName:"__protocol_list"
                                    andSegment:"__OBJC2"];
        if (section_64 == NULL)
            section_64 = [self findSection64ByName:"__objc_protolist"
                                        andSegment:"__DATA"];
        if ((sectionNode = [self
                 findNodeByUserInfo:[self userInfoForSection64:section_64]])) {
            [self createObjC2Pointer64ListNode:sectionNode
        ...
        section_64 = [self findSection64ByName:"__message_refs"
                                    andSegment:"__OBJC2"];
        if (section_64 == NULL)
            section_64 = [self findSection64ByName:"__objc_msgrefs"
                                        andSegment:"__DATA"];
        if ((sectionNode = [self
                 findNodeByUserInfo:[self userInfoForSection64:section_64]])) {
        ...
        section_64 = [self findSection64ByName:"__image_info"
                                    andSegment:"__OBJC"];
        if (section_64 == NULL)
            section_64 = [self findSection64ByName:"__objc_imageinfo"
                                        andSegment:"__DATA"];
        if ((sectionNode = [self
                 findNodeByUserInfo:[self userInfoForSection64:section_64]])) {
            [self
        ...
        if ((sectionNode = [self
                 findNodeByUserInfo:[self userInfoForSection64:section_64]])) {
            [self
                createObjCCFStrings64Node:sectionNode
                                  caption:(lastNodeCaption = @"ObjC CFStrings")
                                 location:section_64->offset + imageOffset
                                   length:section_64->size];
        }
    ...
    for (SectionVector::const_iterator sectIter = ++sections.begin();
        ...
        @try {
            if ((section->flags & S_ATTR_PURE_INSTRUCTIONS) &&
                ...
                [self
                    createTextNode:sectionNode
                           caption:(lastNodeCaption = @"Assembly")
                          location:section->offset + imageOffset
                            length:section->size
                            reloff:section->reloff + imageOffset
                            nreloc:section->nreloc
                         extreloff:dysymtab_command
                                       ? dysymtab_command->extreloff
                                       : 0
                           nextrel:dysymtab_command ? dysymtab_command->nextrel
                                                    : 0
                         locreloff:dysymtab_command
                                       ? dysymtab_command->locreloff
                                       : 0
                           nlocrel:dysymtab_command ? dysymtab_command->nlocrel
                                                    : 0];
            }
        } @catch (NSException *exception) {
            [self printException:exception caption:lastNodeCaption];
    ...
    @try {
        for (Section64Vector::const_iterator sectIter = ++sections_64.begin();
            ...
            if (section_64->nreloc > 0) {
                [self
                    createReloc64Node:relocsNode
                              caption:(lastNodeCaption = [NSString
                                           stringWithFormat:
                                               @"(%s,%s)",
                                               string(section_64->segname, 16)
                                                   .c_str(),
                                               string(section_64->sectname, 16)
                                                   .c_str()])
                             location:section_64->reloff + imageOffset
                               length:section_64->nreloc *
                                      sizeof(struct relocation_info)
...
- (MVNode *)createMachONode:(MVNode *)parent
    ...
    MVNode *node = [parent insertChildWithDetails:caption
                                         location:location
                                           length:sizeof(struct mach_header)
                                            saver:nodeSaver];
    ...
    ]:lastReadHex:@"Magic Number":mach_header->magic == MH_MAGIC
                      ? @"MH_MAGIC"
    ...
    [node.details
        appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"CPU Type":mach_header->cputype == CPU_TYPE_ANY
                      ? @"CPU_TYPE_ANY"
                      : mach_header->cputype == CPU_TYPE_I386
                            ? @"CPU_TYPE_I386"
                            : mach_header->cputype == CPU_TYPE_ARM
                                  ? @"CPU_TYPE_ARM"
                                  : mach_header->cputype == CPU_TYPE_POWERPC
                                        ? @"CPU_TYPE_POWERPC"
                                        : @"???"];

    [dataController read_uint32:range lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"CPU SubType":@""];

    if ((mach_header->cpusubtype & CPU_SUBTYPE_LIB64) == CPU_SUBTYPE_LIB64)
        [node.details appendRow:@"":@"":@"80000000":@"CPU_SUBTYPE_LIB64"];

    if (mach_header->cputype == CPU_TYPE_ARM) {
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_ARM_ALL)
            [node.details appendRow:@"":@"":@"00000000":@"CPU_SUBTYPE_ARM_ALL"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_ARM_V4T)
            [node.details appendRow:@"":@"":@"00000005":@"CPU_SUBTYPE_ARM_V4T"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) == CPU_SUBTYPE_ARM_V6)
            [node.details appendRow:@"":@"":@"00000006":@"CPU_SUBTYPE_ARM_V6"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_ARM_V5TEJ)
            [node.details appendRow:
                                @"":@"":@"00000007":@"CPU_SUBTYPE_ARM_V5TEJ"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_ARM_XSCALE)
            [node.details appendRow:
                                @"":@"":@"00000008":@"CPU_SUBTYPE_ARM_XSCALE"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) == CPU_SUBTYPE_ARM_V7)
            [node.details appendRow:@"":@"":@"00000009":@"CPU_SUBTYPE_ARM_V7"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_ARM_V7F)
            [node.details
                appendRow:
                      @"":@"":@"0000000A":@"CPU_SUBTYPE_ARM_V7F (Cortex A9)"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_ARM_V7S)
            [node.details
                appendRow:
                      @"":@"":@"0000000B":@"CPU_SUBTYPE_ARM_V7S (Swift)"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_ARM_V7K)
            [node.details
                appendRow:
                      @"":@"":@"0000000C":@"CPU_SUBTYPE_ARM_V7K (Kirkwood40)"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_ARM_V6M)
            [node.details appendRow:@"":@"":@"0000000E":@"CPU_SUBTYPE_ARM_V6M"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_ARM_V7M)
            [node.details appendRow:@"":@"":@"0000000F":@"CPU_SUBTYPE_ARM_V7M"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_ARM_V7EM)
            [node.details appendRow:
                                @"":@"":@"00000010":@"CPU_SUBTYPE_ARM_V7EM"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) == CPU_SUBTYPE_ARM_V8)
            [node.details appendRow:@"":@"":@"0000000D":@"CPU_SUBTYPE_ARM_V8"];
    } else if (mach_header->cputype == CPU_TYPE_I386) {
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_I386_ALL)
            [node.details appendRow:
                                @"":@"":@"00000003":@"CPU_SUBTYPE_I386_ALL"];
    } else if (mach_header->cputype == CPU_TYPE_ANY) {
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_MULTIPLE)
            [node.details appendRow:
                                @"":@"":@"FFFFFFFF":@"CPU_SUBTYPE_MULTIPLE"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_LITTLE_ENDIAN)
            [node.details
                appendRow:
                      @"":@"":@"00000000":@"CPU_SUBTYPE_LITTLE_ENDIAN"];
        if ((mach_header->cpusubtype & ~CPU_SUBTYPE_MASK) ==
            CPU_SUBTYPE_BIG_ENDIAN)
            [node.details appendRow:
                                @"":@"":@"00000001":@"CPU_SUBTYPE_BIG_ENDIAN"];
    }
    ...
    ]:lastReadHex:@"File Type":mach_header->filetype == MH_OBJECT
                ? @"MH_OBJECT"
    ...
    [node.details
        appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"Number of Load Commands"
                 :[NSString stringWithFormat:@"%u", mach_header->ncmds]];

    [dataController read_uint32:range lastReadHex:&lastReadHex];
    [node.details
        appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"Size of Load Commands"
    ...
    if (mach_header->flags & MH_NOUNDEFS)
        [node.details appendRow:@"":@"":@"00000001":@"MH_NOUNDEFS"];
    if (mach_header->flags & MH_INCRLINK)
        [node.details appendRow:@"":@"":@"00000002":@"MH_INCRLINK"];
    if (mach_header->flags & MH_DYLDLINK)
        [node.details appendRow:@"":@"":@"00000004":@"MH_DYLDLINK"];
    if (mach_header->flags & MH_BINDATLOAD)
        [node.details appendRow:@"":@"":@"00000008":@"MH_BINDATLOAD"];
    if (mach_header->flags & MH_PREBOUND)
        [node.details appendRow:@"":@"":@"00000010":@"MH_PREBOUND"];
    if (mach_header->flags & MH_SPLIT_SEGS)
        [node.details appendRow:@"":@"":@"00000020":@"MH_SPLIT_SEGS"];
    if (mach_header->flags & MH_LAZY_INIT)
        [node.details appendRow:@"":@"":@"00000040":@"MH_LAZY_INIT"];
    if (mach_header->flags & MH_TWOLEVEL)
        [node.details appendRow:@"":@"":@"00000080":@"MH_TWOLEVEL"];
    if (mach_header->flags & MH_FORCE_FLAT)
        [node.details appendRow:@"":@"":@"00000100":@"MH_FORCE_FLAT"];
    if (mach_header->flags & MH_NOMULTIDEFS)
        [node.details appendRow:@"":@"":@"00000200":@"MH_NOMULTIDEFS"];
    if (mach_header->flags & MH_NOFIXPREBINDING)
        [node.details appendRow:@"":@"":@"00000400":@"MH_NOFIXPREBINDING"];
    if (mach_header->flags & MH_PREBINDABLE)
        [node.details appendRow:@"":@"":@"00000800":@"MH_PREBINDABLE"];
    if (mach_header->flags & MH_ALLMODSBOUND)
        [node.details appendRow:@"":@"":@"00001000":@"MH_ALLMODSBOUND"];
    if (mach_header->flags & MH_SUBSECTIONS_VIA_SYMBOLS)
        [node.details appendRow:
                            @"":@"":@"00002000":@"MH_SUBSECTIONS_VIA_SYMBOLS"];
    if (mach_header->flags & MH_CANONICAL)
        [node.details appendRow:@"":@"":@"00004000":@"MH_CANONICAL"];
    if (mach_header->flags & MH_WEAK_DEFINES)
        [node.details appendRow:@"":@"":@"00008000":@"MH_WEAK_DEFINES"];
    if (mach_header->flags & MH_BINDS_TO_WEAK)
        [node.details appendRow:@"":@"":@"00010000":@"MH_BINDS_TO_WEAK"];
    if (mach_header->flags & MH_ALLOW_STACK_EXECUTION)
        [node.details appendRow:
                            @"":@"":@"00020000":@"MH_ALLOW_STACK_EXECUTION"];
    if (mach_header->flags & MH_ROOT_SAFE)
        [node.details appendRow:@"":@"":@"00040000":@"MH_ROOT_SAFE"];
    if (mach_header->flags & MH_SETUID_SAFE)
        [node.details appendRow:@"":@"":@"00080000":@"MH_SETUID_SAFE"];
    if (mach_header->flags & MH_NO_REEXPORTED_DYLIBS)
        [node.details appendRow:@"":@"":@"00100000":@"MH_NO_REEXPORTED_DYLIBS"];
    if (mach_header->flags & MH_PIE)
        [node.details appendRow:@"":@"":@"00200000":@"MH_PIE"];
    if (mach_header->flags & MH_DEAD_STRIPPABLE_DYLIB)
        [node.details appendRow:
                            @"":@"":@"00400000":@"MH_DEAD_STRIPPABLE_DYLIB"];
    if (mach_header->flags & MH_HAS_TLV_DESCRIPTORS)
...
- (MVNode *)createMachO64Node:(MVNode *)parent
    ...
    MVNode *node = [parent insertChildWithDetails:caption
                                         location:location
                                           length:sizeof(struct mach_header_64)
                                            saver:nodeSaver];
    ...
    [dataController read_uint32:range lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"Magic Number":mach_header_64->magic == MH_MAGIC_64
                                ? @"MH_MAGIC_64"
    ...
    [node.details
        appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"CPU Type":mach_header_64->cputype == CPU_TYPE_ANY
                      ? @"CPU_TYPE_ANY"
                      : mach_header_64->cputype == CPU_TYPE_POWERPC64
                            ? @"CPU_TYPE_POWERPC64"
                            : mach_header_64->cputype == CPU_TYPE_X86_64
                                  ? @"CPU_TYPE_X86_64"
                                  : mach_header_64->cputype == CPU_TYPE_ARM64
                                        ? @"CPU_TYPE_ARM64"
                                        : @"???"];
    ...
    if (mach_header_64->cputype == CPU_TYPE_X86_64) {
        if ((mach_header_64->cpusubtype & ~CPU_SUBTYPE_MASK) ==
    ...
    [node.details
        appendRow:
            [NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"File Type":mach_header_64->filetype == MH_OBJECT
                ? @"MH_OBJECT"
                : mach_header_64->filetype == MH_EXECUTE
                      ? @"MH_EXECUTE"
                      : mach_header_64->filetype == MH_FVMLIB
                            ? @"MH_FVMLIB"
                            : mach_header_64->filetype == MH_CORE
                                  ? @"MH_CORE"
                                  : mach_header_64->filetype == MH_PRELOAD
                                        ? @"MH_PRELOAD"
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location
                           ...
                           :[NSString stringWithFormat:@"%u", mach_header_64->
                                                              sizeofcmds]];
    ...
    if (mach_header_64->flags & MH_NOUNDEFS)
        [node.details appendRow:@"":@"":@"00000001":@"MH_NOUNDEFS"];
    if (mach_header_64->flags & MH_INCRLINK)
        [node.details appendRow:@"":@"":@"00000002":@"MH_INCRLINK"];
    if (mach_header_64->flags & MH_DYLDLINK)
        [node.details appendRow:@"":@"":@"00000004":@"MH_DYLDLINK"];
    if (mach_header_64->flags & MH_BINDATLOAD)
        [node.details appendRow:@"":@"":@"00000008":@"MH_BINDATLOAD"];
    if (mach_header_64->flags & MH_PREBOUND)
        [node.details appendRow:@"":@"":@"00000010":@"MH_PREBOUND"];
    if (mach_header_64->flags & MH_SPLIT_SEGS)
        [node.details appendRow:@"":@"":@"00000020":@"MH_SPLIT_SEGS"];
    if (mach_header_64->flags & MH_LAZY_INIT)
        [node.details appendRow:@"":@"":@"00000040":@"MH_LAZY_INIT"];
    if (mach_header_64->flags & MH_TWOLEVEL)
        [node.details appendRow:@"":@"":@"00000080":@"MH_TWOLEVEL"];
    if (mach_header_64->flags & MH_FORCE_FLAT)
        [node.details appendRow:@"":@"":@"00000100":@"MH_FORCE_FLAT"];
    if (mach_header_64->flags & MH_NOMULTIDEFS)
        [node.details appendRow:@"":@"":@"00000200":@"MH_NOMULTIDEFS"];
    if (mach_header_64->flags & MH_NOFIXPREBINDING)
        [node.details appendRow:@"":@"":@"00000400":@"MH_NOFIXPREBINDING"];
    if (mach_header_64->flags & MH_PREBINDABLE)
        [node.details appendRow:@"":@"":@"00000800":@"MH_PREBINDABLE"];
    if (mach_header_64->flags & MH_ALLMODSBOUND)
        [node.details appendRow:@"":@"":@"00001000":@"MH_ALLMODSBOUND"];
    if (mach_header_64->flags & MH_SUBSECTIONS_VIA_SYMBOLS)
        [node.details appendRow:
                            @"":@"":@"00002000":@"MH_SUBSECTIONS_VIA_SYMBOLS"];
    if (mach_header_64->flags & MH_CANONICAL)
        [node.details appendRow:@"":@"":@"00004000":@"MH_CANONICAL"];
    if (mach_header_64->flags & MH_WEAK_DEFINES)
        [node.details appendRow:@"":@"":@"00008000":@"MH_WEAK_DEFINES"];
    if (mach_header_64->flags & MH_BINDS_TO_WEAK)
        [node.details appendRow:@"":@"":@"00010000":@"MH_BINDS_TO_WEAK"];
    if (mach_header_64->flags & MH_ALLOW_STACK_EXECUTION)
        [node.details appendRow:
                            @"":@"":@"00020000":@"MH_ALLOW_STACK_EXECUTION"];
    if (mach_header_64->flags & MH_ROOT_SAFE)
        [node.details appendRow:@"":@"":@"00040000":@"MH_ROOT_SAFE"];
    if (mach_header_64->flags & MH_SETUID_SAFE)
        [node.details appendRow:@"":@"":@"00080000":@"MH_SETUID_SAFE"];
    if (mach_header_64->flags & MH_NO_REEXPORTED_DYLIBS)
        [node.details appendRow:@"":@"":@"00100000":@"MH_NO_REEXPORTED_DYLIBS"];
    if (mach_header_64->flags & MH_PIE)
        [node.details appendRow:@"":@"":@"00200000":@"MH_PIE"];
    if (mach_header_64->flags & MH_DEAD_STRIPPABLE_DYLIB)
        [node.details appendRow:
                            @"":@"":@"00400000":@"MH_DEAD_STRIPPABLE_DYLIB"];
    if (mach_header_64->flags & MH_HAS_TLV_DESCRIPTORS)
        [node.details appendRow:@"":@"":@"00800000":@"MH_HAS_TLV_DESCRIPTORS"];
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"Reserved":[NSString stringWithFormat:@"%u", reserved]];
    return node;
}
...
- (void)doMainTasks {
    ...
    //=========== Load Commands =============
    {
        ...
        for (uint32_t ncmd = 0; ncmd < ncmds; ++ncmd) {
            ...
            @try {
                [self
                    createLoadCommandNode:commandsNode
                                  caption:(lastNodeCaption = [self
                                               getNameForCommand:load_command->
                                                                 cmd])
                                 location:fileOffset
    ...
    if ([self is64bit] == NO) {
        for (SectionVector::const_iterator sectIter = ++sections.begin();
            ...
            MVNode *sectionNode = [self
                createDataNode:rootNode
                       caption:[NSString
                                   stringWithFormat:@"Section (%s,%s)",
                                                    string(section->segname, 16)
                                                        .c_str(),
            ...
            [sectionNode.userInfo
                addEntriesFromDictionary:[self userInfoForSection:section]];

            NSRange range =
                NSMakeRange(section->reloff + imageOffset,
                            section->nreloc * sizeof(struct relocation_info));
            if (range.length > 0) {
                relocsRange = NSMaxRange(relocsRange) > 0
                                  ? NSUnionRange(relocsRange, range)
                                  : range;
            }
        }
    } else // 64bit
        ...
        for (Section64Vector::const_iterator sectIter = ++sections_64.begin();
            ...
            MVNode *sectionNode = [self
                createDataNode:rootNode
                      ...
                      location:section_64->offset + imageOffset
                        length:(section_64->flags & SECTION_TYPE) ==
                                           S_ZEROFILL ||
                                       (section_64->flags & SECTION_TYPE) ==
                                           S_GB_ZEROFILL
                                   ? 0
                                   : section_64->size];
            ...
            [sectionNode.userInfo
                addEntriesFromDictionary:[self
                                             userInfoForSection64:section_64]];

            NSRange range = NSMakeRange(section_64->reloff + imageOffset,
                                        section_64->nreloc *
                                            sizeof(struct relocation_info));
            if (range.length > 0) {
                relocsRange = NSMaxRange(relocsRange) > 0
...
- (void)doBackgroundTasks {
    NSBlockOperation *linkEditOperation =
    ...
    NSBlockOperation *sectionRelocsOperation =
        [NSBlockOperation blockOperationWithBlock:^{
          if ([backgroundThread isCancelled])
              return;
          @autoreleasepool {
    ...
    NSBlockOperation *dyldInfoOperation =
        [NSBlockOperation blockOperationWithBlock:^{
          if ([backgroundThread isCancelled])
              return;
          @autoreleasepool {
    ...
    NSBlockOperation *sectionOperation =
        [NSBlockOperation blockOperationWithBlock:^{
          if ([backgroundThread isCancelled])
              return;
          @autoreleasepool {
    ...
    NSBlockOperation *EHFramesOperation =
        [NSBlockOperation blockOperationWithBlock:^{
          if ([backgroundThread isCancelled])
              return;
          @autoreleasepool {
    ...
    NSBlockOperation *LSDAsOperation =
        [NSBlockOperation blockOperationWithBlock:^{
          if ([backgroundThread isCancelled])
              return;
          @autoreleasepool {
          ...
          NSLog(@"%@: Lang Spec Data Areas finished parsing. (%lu LSDAs found)",
                self, lsdaInfo.size());
        }];

    NSBlockOperation *objcSectionOperation =
        [NSBlockOperation blockOperationWithBlock:^{
          if ([backgroundThread isCancelled])
              return;
          @autoreleasepool {
    ...
    NSBlockOperation *codeSectionsOperation =
        [NSBlockOperation blockOperationWithBlock:^{
          if ([backgroundThread isCancelled])
              return;
          @autoreleasepool {
    ...
    [codeSectionsOperation
        setThreadPriority:0.0]; // this one will take the longest
    ...
    [oq addOperations:[NSArray
                          arrayWithObjects:linkEditOperation, sectionOperation,
                                           sectionRelocsOperation,
                                           dyldInfoOperation, EHFramesOperation,
                                           LSDAsOperation, objcSectionOperation,
