...
- (BOOL)isOnlyRunningMachOView {
    ...
    for (NSRunningApplication *runningApplication in
        ...
        NSString *fileName =
            [[runningApplication executableURL] lastPathComponent];
...
- (IBAction)attach:(id)sender {
    ...
    if (button == NSAlertFirstButtonReturn) {
        ...
        if ((imagesize = get_image_size(mainAddress, targetPid, &aslr_slide)) ==
            0) {
        ...
        [self application:NSApp
                 openFile:[NSString stringWithCString:dumpFilePath
...
- (void)applicationWillFinishLaunching:(NSNotification *)aNotification {
    ...
    //  @"UseLLVMDisassembler"] != nil)
    //    qflag = [[NSUserDefaults standardUserDefaults]
    //    boolForKey:@"UseLLVMDisassembler"];
    ...
    if (isFirstMachOView && [fileManager fileExistsAtPath:tempDir
                                              isDirectory:NULL] == YES) {
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    [NSThread detachNewThreadSelector:@selector(printStat)
                             toTarget:self
                           withObject:nil];
#endif
