...
static AsmFootPrint const SDK104Target104X86v1 = {
    ...
    { 2, 0x8D, 0x83 },
    GAP(4),                // lea     eax, (a__dyld_mod_ter - 2C28h)[ebx] ;
    ...
    { 4, 0x89, 0x44, 0x24, 0x04}, // mov     [esp+4], eax
    { 2, 0x8D, 0x83 },
    GAP(4),            // lea     eax, (a__dyld_make_de - 2D25h)[ebx] ;
                       // "__dyld_make_delayed_module_initializer_"...
    { 3, 0x89, 0x04, 0x24}, // mov     [esp], eax
...
static AsmFootPrint const SDK104Target104X86v2 = {
    ...
    { 2, 0xEB,
      0x19 }, // jmp     short loc_73
              // ---------------------------------------------------------------------------

    // loc_5A:
    { 2, 0x89, 0xCA }, // mov     edx, ecx
    { 2, 0xEB,
      0x0E }, // jmp     short loc_6C
              // ---------------------------------------------------------------------------
...
static AsmFootPrint const SDK104Target104X86_64 = {
    ...
    { 2, 0xEB,
      0x1D }, // jmp     short loc_7C
              // ---------------------------------------------------------------------------

    // loc_5F
    { 3, 0x48, 0x89, 0xCA}, // mov     rdx, rcx
    { 2, 0xEB,
      0x11 }, // jmp     short loc_75
    // ---------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
- (bool)matchAsmAtOffset:(NSUInteger)offset
...
- (void)determineRuntimeVersion {
    ...
    NSUInteger dataLength = [dataController.fileData length];
    ...
    if ([self is64bit] == NO) {
        if (MATCHASM(SDK104Target104X86v1)) {
            ...
            rootNode.caption = [rootNode.caption
                                stringByAppendingString:@" [SDK10.4 Target10.4]"];
            return;
            ...
            rootNode.caption = [rootNode.caption
                                stringByAppendingString:@" [SDK10.5 Target10.4]"];
            return;
        } else if (MATCHASM(SDK105Target105X86)) {
            NSLog(@"SDK105Target105X86 matched");
            rootNode.caption = [rootNode.caption
                                stringByAppendingString:@" [SDK10.5 Target10.5]"];
            return;
        } else if (MATCHASM(SDK106Target104X86) ||
                   MATCHASM(SDK106Target104X86v2)) {
            NSLog(@"SDK106Target104X86 matched");
            rootNode.caption = [rootNode.caption
                                stringByAppendingString:@" [SDK10.6 Target10.4]"];
            return;
            ...
            for (CommandVector::const_iterator cmdIter = commands.begin();
                ...
                if (load_command->cmd == LC_DYLD_INFO_ONLY) {
                    rootNode.caption = [rootNode.caption
                                        stringByAppendingString:@" [SDK10.6 Target10.6]"];
                    NSLog(@"LC_DYLD_INFO_ONLY  ==> target10.6");
                    return;
                }
            }
            rootNode.caption = [rootNode.caption
                                stringByAppendingString:@" [SDK10.6 Target10.5]"];
            return;
        } else if (MATCHASM(SDK109Target104X86)) {
            NSLog(@"SDK109Target104X86 matched");

            rootNode.caption = [rootNode.caption
                                stringByAppendingString:@" [SDK10.9 Target10.4]"];
            return;
        }
    } else {
        if (MATCHASM(SDK104Target104X86_64)) {
            ...
            for (CommandVector::const_iterator cmdIter = commands.begin();
                ...
                if (load_command->cmd == LC_DYLD_INFO_ONLY) {
                    NSLog(@"LC_DYLD_INFO_ONLY  ==> target10.6");
                    rootNode.caption = [rootNode.caption
                                        stringByAppendingString:@" [SDK10.6]"];
                    return;
                }
            }
            rootNode.caption =
                [rootNode.caption stringByAppendingString:@" [SDK10.5]"];
            return;
        }
    }
}
