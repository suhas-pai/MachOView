...
NSString *const MVDataTreeWillChangeNotification =
    @"MVDataTreeWillChangeNotification";
NSString *const MVDataTreeDidChangeNotification =
    @"MVDataTreeDidChangeNotification";
...
//-----------------------------------------------------------------------------
+ (MVColoumns *)coloumnsWithData:(NSString *)col0
                                :(NSString *)col1
                                :(NSString *)col2
                                :(NSString *)col3 {
...
- (void)writeColor:(NSColor *)color toFile:(FILE *)pFile {
    int colorOrdinal =
        [color isEqualTo:[NSColor blackColor]]
            ? MVBlackColorOrdinal
            : [color isEqualTo:[NSColor darkGrayColor]]
                  ? MVDarkGrayColorOrdinal
                  : [color isEqualTo:[NSColor lightGrayColor]]
                        ? MVLightGrayColorOrdinal
                        : [color isEqualTo:[NSColor whiteColor]]
                              ? MVWhiteColorOrdinal
                              : [color isEqualTo:[NSColor grayColor]]
                                    ? MVGrayColorOrdinal
...
- (NSColor *)readColorFromFile:(FILE *)pFile {
    ...
    return [NSColor colorWithDeviceRed:fred
                                 green:fgreen
                                  blue:fblue
                                 alpha:falpha];
}
...
- (void)saveAttributestoFile:(FILE *)pFile {
    ...
    for (NSString *key in [attributes allKeys]) {
        ...
        int keyOrdinal =
            [key isEqualToString:MVUnderlineAttributeName]
                ? MVUnderlineAttributeOrdinal
                : [key isEqualToString:MVCellColorAttributeName]
                      ? MVCellColorAttributeOrdinal
                      : [key isEqualToString:MVTextColorAttributeName]
                            ? MVTextColorAttributeOrdinal
                            : [key isEqualToString:MVMetaDataAttributeName]
                                  ? MVMetaDataAttributeOrdinal
                                  : 0;
...
- (void)loadAttributesFromFile:(FILE *)pFile {
    ...
    NSMutableDictionary *_attributes =
        [[NSMutableDictionary alloc] initWithCapacity:numAttributes];
...
//----------------------------------------------------------------------------
- (void)updateCellContentTo:(id)object
                      atRow:(NSUInteger)rowIndex
                     andCol:(NSUInteger)colIndex {
...
- (void)applyFilter:(NSString *)filter {
    ...
    if (filter == nil || [filter length] == 0) {
        // copy everything (copy by elems because want to exclude later added
        // rows)
        ...
        for (MVRow *row in rows) {
            ...
            NSString *metadata =
                [row.attributes objectForKey:MVMetaDataAttributeName];
            if (metadata == nil ||
                [predicate evaluateWithObject:metadata] == YES) {
                [displayRows addObject:row];
            }
        }
    }
...
- (void)dealloc
{
  ...
  {
    NSLog(@"%@ Retain count is %ld", layout, CFGetRetainCount((__bridge
CFTypeRef)layout));
  }
}
*/
...
- (void)createMachOLayout:(MVNode *)node
    ...
    MachOLayout *layout = [MachOLayout layoutWithDataController:self
                                                       rootNode:node];
...
- (void)createArchiveLayout:(MVNode *)node machine:(NSString *)machine {
    node.caption =
        machine ? [NSString stringWithFormat:@"Static Library (%@)", machine]
                : @"Static Library";

    ArchiveLayout *layout = [ArchiveLayout layoutWithDataController:self
                                                           rootNode:node];
...
- (void)createFatLayout:(MVNode *)node
    ...
    for (uint32_t nimg = 0; nimg < fat_header->nfat_arch; ++nimg) {
        ...
        MVNode *archNode = [node insertChild:nil
                                    location:fat_arch.offset
                                      length:fat_arch.size];

        if (*(uint64_t *)((uint8_t *)[fileData bytes] + fat_arch.offset) ==
            *(uint64_t *)"!<arch>\n") {
            [self createArchiveLayout:archNode
                              machine:[self getMachine:fat_arch.cputype]];
        } else {
            [self createLayouts:archNode
                       location:fat_arch.offset
                         length:fat_arch.size];
        }
    }
...
- (void)treeViewWillChange {
    NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
    [nc postNotificationName:MVDataTreeWillChangeNotification object:self];
}

//----------------------------------------------------------------------------
- (void)treeViewDidChange {
    NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
    [nc postNotificationName:MVDataTreeDidChangeNotification object:self];
}
...
- (void)updateTableView {
    NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
    [nc postNotificationName:MVDataTableChangedNotification object:self];
}
...
- (id)initWithPath:(NSString *)path {
    if (self = [super init]) {
        ...
        if (pFile == NULL) {
            NSLog(@"*** file cannot be created: %@ '%s'", swapPath,
                  strerror(errno));
            return nil;
        }
        ...
        saverThread = [[NSThread alloc] initWithTarget:self
...
{
    NSParameterAssert([object conformsToProtocol:@protocol(MVSerializing)] ==
                      YES);
    ...
    // if the background saver thread has been cancelled, then do do one cycle
    // manually
...
#if DEBUG
                NSLog(@"%@: saving %lu rows", [NSThread currentThread],
                      (unsigned long)[objectsToSave count]);
#endif
