...
static kern_return_t readmem(mach_vm_offset_t *buffer,
                             ...
                             pid_t pid, vm_region_basic_info_data_64_t *info);
...
kern_return_t find_main_binary(pid_t pid, mach_vm_address_t *main_address) {
    ...
    if (task_for_pid(mach_task_self(), pid, &targetTask)) {
        NSLog(@"[ERROR] Can't execute task_for_pid! Do you have the right "
    ...
    while (1) {
        ...
        if (vm_region_recurse_64(targetTask, &addr, &lsize, &depth,
                                 (vm_region_info_t)&info, &count)) {
            break;
        }
        kr = mach_vm_read_overwrite(targetTask, (mach_vm_address_t)addr,
 ...
 */
int64_t get_image_size(mach_vm_address_t address, pid_t pid,
    ...
    if (readmem((mach_vm_offset_t *)&header, address,
                sizeof(struct mach_header), pid, &region_info)) {
    ...
    if (readmem((mach_vm_offset_t *)loadcmds, address + mach_header_size,
                header.sizeofcmds, pid, &region_info)) {
 ...
 */
kern_return_t dump_binary(mach_vm_address_t address, pid_t pid, uint8_t *buffer,
                          uint64_t aslr_slide) {
    vm_region_basic_info_data_64_t region_info = {0};
    // allocate a buffer to read the header info
    // NOTE: this is not exactly correct since the 64bit version has an extra 4
        ...
        if (loadCommand->cmd == LC_SEGMENT) {
            ...
            if (strncmp(segCmd->segname, "__PAGEZERO", 16) != 0) {
                ...
                printf("[DEBUG] Dumping %s at %llx with size %x (buffer:%x)\n",
                       segCmd->segname, segCmd->vmaddr + aslr_slide,
                       segCmd->filesize, (uint32_t)*buffer);
#endif
                readmem((mach_vm_offset_t *)buffer, segCmd->vmaddr + aslr_slide,
                        segCmd->filesize, pid, &region_info);
            }
...
#if DEBUG
                printf(
                    "[DEBUG] Dumping %s at %llx with size %llx (buffer:%x)\n",
                    segCmd64->segname, segCmd64->vmaddr + aslr_slide,
                    segCmd64->filesize, (uint32_t)*buffer);
#endif
                readmem((mach_vm_offset_t *)buffer,
                        segCmd64->vmaddr + aslr_slide, segCmd64->filesize, pid,
                        &region_info);
            }
...
static kern_return_t readmem(mach_vm_offset_t *buffer,
    ...
    kern_return_t kr;
    //#if DEBUG
    //    printf("[DEBUG] Readmem of address %llx to buffer %llx with size
    //    %llx\n", address, buffer, size);
    //#endif
    if (task_for_pid(mach_task_self(), pid, &port)) {
        fprintf(stderr, "[ERROR] Can't execute task_for_pid! Do you have the "
    ...
    kr =
        mach_vm_region(port, &address_info, &size_info, VM_REGION_BASIC_INFO_64,
