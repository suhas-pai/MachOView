...
#define ENCODING_FIXSIZE(format)                                               \
    ((format & 0xf) == DW_EH_PE_udata2 || (format & 0xf) == DW_EH_PE_sdata2)   \
        ? sizeof(uint16_t)                                                     \
        : ((format & 0xf) == DW_EH_PE_udata4 ||                                \
           (format & 0xf) == DW_EH_PE_sdata4)                                  \
              ? sizeof(uint32_t)                                               \
              : ((format & 0xf) == DW_EH_PE_udata8 ||                          \
                 (format & 0xf) == DW_EH_PE_sdata8)                            \
                    ? sizeof(uint64_t)                                         \
                    : ((format & 0xf) == DW_EH_PE_uleb128 ||                   \
                       (format & 0xf) == DW_EH_PE_sleb128)                     \
                          ? 0                                                  \
                          : ([self is64bit] == NO) ? sizeof(uint32_t)          \
                                                   : sizeof(uint64_t)

//-----------------------------------------------------------------------------
#define READ_USE_ENCODING(format, range, hexstr)                               \
    ((format & 0xf) == DW_EH_PE_udata2 || (format & 0xf) == DW_EH_PE_sdata2)   \
        ? [dataController read_uint16:range lastReadHex:&hexstr]               \
        : ((format & 0xf) == DW_EH_PE_udata4 ||                                \
           (format & 0xf) == DW_EH_PE_sdata4)                                  \
              ? [dataController read_uint32:range lastReadHex:&hexstr]         \
              : ((format & 0xf) == DW_EH_PE_udata8 ||                          \
                 (format & 0xf) == DW_EH_PE_sdata8)                            \
...
- (NSString *)guessSymbolUsingEncoding:(uint8_t)format
    ...
    // NSLog(@"guessed at %X [%X]: %@", [self fileOffsetToRVA:offset], value,
    // symbolName);
...
- (MVNode *)createCFINode:(MVNode *)parent
    ...
    // structure, not including the Length and Extended Length fields.
    ...
    // records. This value shall always be 0, which indicates this record is a
    // CIE.
    uint32_t CIE_ID = [dataController read_uint32:range
                                      lastReadHex:&lastReadHex];
    ...
    // shall be 1.
    uint8_t CIE_version = [dataController read_uint8:range
                                         lastReadHex:&lastReadHex];
    CIE_length -= range.length;
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ...
    NSString *CIE_augmentationStr = [dataController read_string:range
                                                    lastReadHex:&lastReadHex];
    CIE_length -= range.length;
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ...
    // architectures, this is a 8 byte value that...
    NSAssert([CIE_augmentationStr rangeOfString:@"eh"].location == NSNotFound,
             @"EH Data field is present");

    // Code Alignment Factor (Required)
    // An unsigned LEB128 encoded value that is factored out of all advance
    // location instructions that are associated with this CIE or its FDEs. This
    // value shall be multiplied by the delta argument of an adavance location
    // instruction to obtain the new location value.
    uint64_t CIE_codeAlignFactor = [dataController read_uleb128:range
                                                    lastReadHex:&lastReadHex];
    CIE_length -= range.length;
    [node.details
    ...
    int64_t CIE_dataAlignFactor = [dataController read_sleb128:range
                                                   lastReadHex:&lastReadHex];
    CIE_length -= range.length;
    [node.details
     appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ...
    // Augmentation Data shall be intepreted according to other characters in
    // the Augmentation String.
    if ([CIE_augmentationStr rangeOfString:@"z"].location != NSNotFound) {
        // Augmentation Data Length (Optional)
        // An unsigned LEB128 encoded value indicating the length in bytes of
        // the Augmentation Data. This field is only present if the Augmentation
        // String contains the character 'z'.
        uint64_t CIE_augmentationLength =
        ...
        // Augmentation String as described below. This field is only present if
        // the Augmentation String contains the character 'z'.
    ...
    [dataController read_bytes:range
                        length:CIE_length
                   lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ...
    if (Pointer_encoding != DW_EH_PE_omit)
        while (NSMaxRange(range) - location < length) {
            ...
            // Length	(Required)
            // A 4 byte unsigned value indicating the length in bytes of the CIE
            // structure, not including the Length field itself. If Length
            // contains the value 0xffffffff, then the length is contained the
            // Extended Length field. If Length contains the value 0, then this
            ...
            // the current FDE yields the offset of the start of the associated
            // CIE. This value shall never be 0.
            uint32_t FDE_CIEvalue = [dataController read_uint32:range
                                                    lastReadHex:&lastReadHex];
            FDE_length -= range.length;

            // we ran out of FDE records and started to process the forthcoming
            // CIE !! rollback until the begining of the CIE
            if (FDE_CIEvalue == 0) {
                [node.details popRow];
                break;
            }

            uint64_t FDE_CIEpointer =
                ([self is64bit] == NO
            ...
            if ([CIE_augmentationStr rangeOfString:@"z"].location !=
                NSNotFound) {
                // Augmentation Data Length	(Optional)
                // An unsigned LEB128 encoded value indicating the length in
                // bytes of the Augmentation Data. This field is only present if
                // the Augmentation String in the associated CIE contains the
                // character 'z'.
                uint64_t FDE_augmentationLength =
                    [dataController read_uleb128:range
                                     lastReadHex:&lastReadHex];
                FDE_length -= range.length;
                [node.details
                ...
                if (FDE_augmentationLength > 0) { // LSDA is present
                    ...
                    // as described above. This field is only present if the
                    // Augmentation String in the associated CIE contains the
                    // character 'z'.
                    uint64_t LSDA_addr =
 ...
 *  .byte       @TType format       (usually <omit>, <absolute> or <indirect
 * pcrel sdata4>) .uleb128    @TType base offset  (optional depending on TType
 * format) .byte       Call-site format .uleb128    Call-site table length
 *
 *  #call-site table
 *  .long       region 0 start
 *  .long       length
...

{
    ...
    //==================== Call Sites Table ====================
    NSAssert(callSiteFormat == DW_EH_PE_udata4,
             @"Not yet implemeted encoding for call-site format");
    ...
    if (typeTableFormat != DW_EH_PE_omit) {
        ...
        for (IndexSet::iterator iter = exceptionSpecs.begin();
             iter != exceptionSpecs.end(); ++iter) {
            int32_t index = *iter;

            NSRange range = NSMakeRange(typeTableBaseLocation - index - 1, 0);

            // Negative value, starting at -1, which is the byte offset in the
            // types table of a null-terminated list of type indexes. The list
            // will be at TTBase+1 for -1, at TTBase+2 for -2, and so on. Used
            // by the runtime to match the type of the thrown exception with the
        ...
        // traverse type filters in reverse order (starting with the catch
        // clauses)
        for (IndexSet::reverse_iterator iter = typeIndexes.rbegin();
             iter != typeIndexes.rend(); ++iter) {
            int32_t index = *iter;
            ...
            // preceding TTBase, 2 is the second word, and so on. Used by the
            // runtime to check if the thrown exception type matches the
            // catch-clause type.
...
- (MVNode *)createUnwindInfoHeaderNode:(MVNode *)parent
    ...
    MVNode *node =
        [parent insertChildWithDetails:caption
                              location:location
                                length:sizeof(struct unwind_info_section_header)
                                 saver:nodeSaver];
    ...
    NSAssert1(version == UNWIND_SECTION_VERSION,
              @"unsupported unwind section version (%u)", version);
