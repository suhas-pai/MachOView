...
- (MVNode *)createSignatureNode:(MVNode *)parent
    ...
    NSString *signature = [dataController read_string:range
                                               fixlen:8
                                          lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"Signature":signature];
    return node;
}
//----------------------------------------------------------------------------
...
- (MVNode *)createHeaderNode:(MVNode *)parent
    ...
    NSString *time_str = [dataController read_string:range
                                              fixlen:12
                                         lastReadHex:&lastReadHex];
    ...
    NSString *user_id_str = [dataController read_string:range
                                                 fixlen:6
                                            lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"UserID":[NSString stringWithFormat:@"%u",
                                                       [user_id_str intValue]]];

    NSString *group_id_str = [dataController read_string:range
                                                  fixlen:6
                                             lastReadHex:&lastReadHex];
    [node.details
    ...
    NSString *mode_str = [dataController read_string:range
                                              fixlen:8
                                         lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"Mode":[NSString
                              stringWithFormat:@"%u", [mode_str intValue]]];

    NSString *size_str = [dataController read_string:range
                                              fixlen:8
                                         lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"Size":[NSString
                              stringWithFormat:@"%u", [size_str intValue]]];

    // read spaces until end-of-header (0x60 0x0A)
    NSMutableString *mutableLastReadHex =
        [[NSMutableString alloc] initWithCapacity:2];
    NSMutableString *padding = [[NSMutableString alloc] initWithCapacity:2];
    for (;;) {
        [padding appendString:[dataController read_string:range
                                                   fixlen:1
                                              lastReadHex:&lastReadHex]];
    ...
    [node.details
        appendRow:[NSString stringWithFormat:@"%.8lX", range.location -
                                                           [padding length] + 2
    ]:mutableLastReadHex:@"End Header":padding];
    ...
    if (NSEqualRanges([name rangeOfString:@"#1/"], NSMakeRange(0, 3))) {
        ...
        NSString *long_name = [dataController read_string:range
                                                   fixlen:len
                                              lastReadHex:&lastReadHex];
        [node.details
            appendRow:[NSString stringWithFormat:@"%.8lX", range.location
        ]:lastReadHex:@"Long Name":long_name];

        objectInfo =
            [MVObjectInfo objectInfoWithName:long_name
                                      Length:[size_str intValue] - len];
    } else {
        objectInfo = [MVObjectInfo objectInfoWithName:name
                                               Length:[size_str intValue]];
    }
    [objectInfoMap setObject:objectInfo
                      forKey:[NSNumber numberWithUnsignedLong:location]];
...
- (MVNode *)createMemberNode:(MVNode *)parent
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location
    ]:lastReadHex:@"Size":[NSString stringWithFormat:@"%u", size]];
    ...
    while (size > 0) {
        uint32_t strx = [dataController read_uint32:range
                                        lastReadHex:&lastReadHex];
        ...
        [node.details
            appendRow:[NSString stringWithFormat:@"%.8lX", range.location
        ]:lastReadHex:@"Symbol":symbolName];

        uint32_t off = [dataController read_uint32:range
                                       lastReadHex:&lastReadHex];

        MVObjectInfo *objectInfo = [objectInfoMap
            objectForKey:[NSNumber numberWithUnsignedLong:off + imageOffset]];
...
- (void)doBackgroundTasks {
    ...
    for (MVObjectInfo *objectInfo in [objectInfoMap allValues]) {
        ...
        // if the thread is cancelled, then the MVLayout::doBackgroundTasks will
        // not been called so, here is the only chance to stop the saver thread
